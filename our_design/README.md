# Lesson: Digital Storytelling
# Design Document

### First and Last Name: Αναστασιάδου Μαρίνα-Ελένη , Παντελοπούλου Σοφία 
### University Registration Number: dpsd19005, dpsd19140
### GitHub Personal Profile: https://github.com/Marina-Eleni-Anastasiadou , https://github.com/DPSD19140
## Team Name : The Burned Cells

<p>
  
  
  <p>
    <strong>1.ΥΠΟΘΕΣΗ ΚΑΙ ΣΤΟΧΟΣ </strong> 
    </p>
    
 <p>    Η υπόθεση περιστρέφεται  γύρω απο την  φίλια.
     Το κύριο  θέμα μας είναι η  αναπτύξη της φίλιας στα χρόνια του πανέπηστημιου απο την αρχή της  γνώριμιας στο πρώτο έτος. 
     Η υπόθεση της ταινίας διαδραμάτιζει την  αναδρόμη της φιλίας των 2 κορίτσιων με την απόστολη ένος γράμματος η τελευταιία απο τος 2 που απέμειναν στο νήσι στην άλλη η οπόια έχει φύγει έδω και κάποιους μήνες.
     Πράκτικα θέλουμε να δείξουμε πως κάτα την διάρκεια των χρόνων δήμιουργηθηκε ένα συναισθηματικό δέσιμο μέταξυ 2 φοιτήτριων και 
     το πως άυτο δημιουργήθηκε.Η ταίνια βασίζεται σε άληθινα γεγόνοτα και έχει σκοπό να μετάδωσει αύτην την εμπείρια στον θεάτη. 
     </p>
  <p>
  <strong>2. ΜΕΣΑ ΑΦΗΓΗΣΗΣ</strong>
     </p>
     
   <p>  Για την αφήγηση αυτής της ιστορίας χρησιμοποιήσαμε δύο διαφορετικά μέσα αφήγησης. Το ένα που χρησιμοποιήσαμε ήταν η παραγωγή μιας ταινίας μικρού μήκους για να αποδώσει στιγμές της καθημερινότητας των δύο φίλων . Το άλλο ήταν η παραγωγή ενός επιτραπέζιου παιχνιδιού το οποίο συνδυάζεται με την ταινία και προσθέτει λεπτομέρειες από  τα ταξίδια και τις εμπειρίες που πέρασαν οι δύο φίλες
     </p> 
   <p>
  <strong>3. CASE</strong>
         </p>
         <p> Η υπόθεση του έργου μας είναι η αφήγηση ενός γράμματος που στέλνει η μία φίλη στην άλλη. Αυτή που στέλνει το γράμμα είναι στο νησί της Σύρου για να  τελευταία φορά καθώς ετοιμάζεται να αποφοιτήσει και η άλλη έχει φύγει προ πολλού. 
      Οπότε κάνοντας βόλτα για να  συγκράτησει το νησί στη μνήμη της ,θυμήθηκε τη στιγμή της γνωριμίας τους καθώς και πολλά άλλα πράγματα που βίωσαν μαζί και αποφάσισε να τις της εκφράσει με τη μορφή ενός γράμματος. 
      Το οποίο στη συγκεκριμένη  ταινία συνδυάζεται με βίντεο από μέρη τα οποία αναφέρονται στο γράμμα.</p>
   
   <p><strong>4. WORLDBUILDING </strong>
         </p>     
      <p>   Η ταινία δραματική σε πραγματικό χώρο και σε παρελθοντικό χρόνο σε διάφορα μέρη της  Σύρου ,της Αθήνας ,της Θεσσαλονίκης ,της Πάρου ,της Τήνου και της Δράμας στο χρονικό περιθώριο 2019 με 2025.
         </p>
   <p>
  <strong>5. ΧΑΡΑΚΤΗΡΕΣ</strong>
         </p>  
         <p>
         Πρακτικά δεν υπάρχει πρωταγωνιστής παρά μόνο αναφέρονται οι δύο κοπέλες Η μία εκ της αφήγησης του γράμματος και οι άλλοι μέσα από το περιεχόμενο του.
        </p>
        
 <p>
  <strong>6. ΑΚΟΛΟΥΘΙΑ ΣΚΗΝΩΝ</strong>
         </p> 
         <p>
  
- Θάλασσα 
* Όμπρελα παραλίας από κάτω όψη 
+ Κτίριο 1 Γυμνάσιου Σύρου
- Εσώτερικες σκάλες κτιρίου
* Δημαρχείο Σύρου 
+ Παγκάκι 
- Θάλασσα
* Λίμανι (μέγαλο)
+ Γάτες
- Γάτα μόνη της  
* Los gatitos
+ Παράλια Γαλλησά
- Θαλασσά
* Πάρκο
+ Στενό σκάλες
- Δρόμος το βράδυ 
* Στένο με βουκαμβίλια
+ Στένο δρόμος
- Μπαζαρ βιβλίων
* Βιβλία στην σείρα 
+ Λίμανι (μίκρο)
- Πόλη το βράδυ  
* Ουράνος 
+ Βένζινάδικο
- Θαλασσά
* Θαλασσά
+ Λουλούδια
- Βραχονησίδα
* Τοπίο τήνου
+ Λίμανι (αντίπαρου)
- Στένο δρόμος
* Στένο άγορα 
+ Παλαίο κτήριο
- Παλαιό κτήριο
* Δρόμος κάτω όψη
+ Παράλια Αστέρια 
- Εσωτέρικο σπίτιου 
* Παζλ πάνω όψη 
+ Τοίχος με αφίσες
- Εσωτερικό μέγαρου μούσικης άνω όψη
* Τάμπελα μέτρο  
+ Εσωτερικό στραύρος νιάρχος
- Μουσείο Ακρόπολης είσοδος 
* Μουσείο Ακρόπολης εκθέματα
+ Πλόιο που φέυγει
- Νυχτέρινος ουράνος 
* Σκήνη πόλης
+ Λέωφορειο
- Φύτα σπίτιου
* Εσωτερίκη σκήνη κτιρίου
+ Πλοίο εσώτερικο
- Θέσης πλοίου 
* Σκιες

      
  </p>
 <p>
  <strong>7. ΤΟΠΟΘΕΣΙΕΣ ΚΙΝΗΜΑΤΟΓΡΑΦΗΣΗΣ</strong>
         </p> 
         <p> Η ταινία μας γυρίστηκε  στην Σύρο , Άθηνα ,Πάρο ,Τήνο ,Δράμα ,Θεσσαλονίκη και πιο συγκέκριμενα για την κάθε μια :

 #### 1. Σύρος
   - Πλάτεια Μιάουλη
   + Κτήριο 1 γυμνάσιου
   * Επιμελητίριο κυκλάδων
   - Λίμανι ερμούπολης
   + Λαζαρέτα 
   * Λιμάνακι στα λαζαρέτα 
   - Αγίος νικόλαος
   + Αγόρα 
   * Αστέρια
   - Γαλησσάς
   
  #### 2. Αθήνα
   - Εξάρχεια 
   + Μέγαρο Μουσίκης Αθήνων 
   
  #### 3. Πάρο
   * Λίμανι 
   - Αντίπαρος
   + Θάλασσα
  
   #### 4. Τήνος
   * Δύο χώρια
   
   #### 5. Δράμα
   * Ανθώπολειο
   
   #### 6. Θεσσάλονικη
   * Κτελ Μακέδονιας
   
</p>
         
 <p>
  <strong>8. ΑΙΣΘΗΤΙΚΗ</strong>
         </p> 
         
       
H αισθητίκη της ταινίας μας κυρίως στα φυσίκα χρώματα των τόπιων τα όποια τόνιζουμε ελάφρα με την χρήση φίλτρων. Οι αποχρώσεις τους είνια κυρίως ζεστές για τόνησουν την θεματίκη μας.

   
![3d3c9628f69a282523db9e77dcba3e9f](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/790ec992-5841-4f92-bdc0-287dc3dd7e1b)

 ![Screenshot 2023-05-31 222542](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/2f5caa58-f006-49d6-9618-24a66147a3e3)

![Screenshot 2023-05-31 222706](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/52e543a6-70ed-4e0b-bc4e-c40eb6873cc9)


         
 <p>
  <strong>9. ΣΚΗΝΙΚΑ </strong>
         </p>     
    <p>Υπάρχει η αφήγηση με την οπόια η πρωτάγονιστρια διαβάζει στο κοίνο το γράμμα τπρος την φίλη της .(εδω  θα μπει το γράμμα)</p>
       
         
 <p>
  <strong>10. ΠΡΟΥΠΟΛΟΓΙΣΜΟΣ ΠΑΡΑΓΩΓΗΣ ΚΑΙ ΧΡΗΜΑΤΟΔΟΤΗΣΗ</strong>
         </p>   
      <p>   
         Δεδόμενου ότι όλα τα πλάνα τραβήχτηκαν με την βόηθεια κάμερας κίνητου τηλεφωνου , το editing  & μοντάζ έγινε μέσω ένος υπολόγιστη  και η ηχογραφήση μέσω 
         μικροφώνου οπόυ έιναι προσώπικα μας αντίκειμενα το κόστος ήταν μηδαμίνο.
         </p>
         
 <p>
  <strong>11. ΠΡΟΒΟΛΗ/ΠΕΡΙΒΑΛΛΟΝ ΑΛΛΗΛΕΠΙΔΡΑΣΗΣ</strong>
         </p>
Η ταινία θα προβλήθει στην  online πλατφόρμα του youtube. 
<p>To επιτράπεζιο παιχνίδι το οποίο θα συνόδεβει την ταινιά ,εμπλουτίζει την ίστορια κάθως και περίεχει μίκρες λέπτόμεριες που δεν μπόρεσαμε να βάλουμε στην ταίνια για αυτό και δεν μπόρει να αξιόποιηθει ως αυτόνομο μέσο.</p>

 <p>
  <strong>12. ΑΠΕΥΘΥΝΟΜΕΝΟ ΚΟΙΝΟ & ΑΓΟΡΑ</strong>
         </p>
         <p>Το συγκεκρίμρνο project κύριως απευθύνεται σε κοίνο 13 ετών και πάνω λόγο θέματόλογιας.
  Οι θεατές μπόρει να έχουν βιώσει κατί παρόμοιο ή και να θέλουν.  </p>
 <p>
  <strong>13. ΕΞΟΠΛΙΣΜΟΣ</strong>
         </p>    

   #### Για ταινία      
- Kίνητα 
* Υπόλογιστης
+ Μικρόφωνο
  
  #### Για επιτράπεζιο   παιχνίδι
- Μακέτοχαρτο
+ Ακρύλικες μπόγιες
* Μαύρα πενάκια 
- Χάρακες 
+ Πολύχρωμα χαρτόνια
* Ζάρια

 
  
 <p><strong>14. ΧΡΟΝΟΔΙΑΓΡΑΜΜΑ</strong>
         </p> 
       
 <p>  1. Επίλογη Θέματος</p>
 <p>  2. Δημίουργια γράμματος</p>
 <p>  3. Λήψη Πλάνων </p>
 <p>  4. Έξτρα μέσο αφήγησης </p>
 <p>  5. Μοντάζ </p>
 <p>  6. Ηχόγραφηση</p>
         
 <p><strong>15. ΜΕΣΑ ΔΙΑΝΟΜΗΣ</strong>
         </p>  
   
   #### Ταινία μικρού μήκους
 
 <p>   Μέσο αυτής απώδειδεται το συναισθήμα της αλήθινης φίλιας κάθως και της νοστάλγιας για να μπόρεσει να ταυτίστει ο θέατης.
    Απότελει το πρώτο μέσο αφήγησης , δίνει στον θεάτη περίσσοτερα οπτίκο ακούστηκα ερεθίσματα για να κατάλαβει καλύτερα τον χώρο και τον χρόνικο πλαίσιο στο οποίο διαδράματιζεται.
    Το γράμμα της μιας φίλης στην άλλη που έφυγε είναι ο πύρηνας της ιστόριας μας.Η πρωταγωνίστρια καλείται να εκφράσει την νοσταλγία που έχει για τις ωραιες στογμές με την φίλη της και παράλληλα να της το πόσο πόλυ την νοίαζεται και ανύποονει να την ξαναδεί. 
 </p>  
         
   #### Επιτραπέζιο παιχνίδι 
   
   <p> Με την χρήση αυτόυ δόμθηκε ένας μίκρος χαρτής που δέιχνει μερίκα άπο τα μέρη που ταξίδευυαν οι δυο φίλες και απέκτισαν πόλλες άναμνησεις. Ο συνδιασμός της ταινίας με το παιχνίιδι δίνει την δυνατότητα στον θεατή να κατανόησει το βάθος της σχέσης των δυο κορίτσιων και να ταυτίστει καλύτερα μαζί τους. </p>
   
![1685606239473](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/df95f308-f600-4a1d-b4f8-c65d15462c0c)
         
![1685606239460](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/ef7b56e3-0f3b-4e4e-a4dd-8636ac0c0f36)

![1685606239467](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/4a465bd5-a0df-4ec1-bbb9-2e9ba59b98ee)

![1685606812497](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/46bd42d3-3593-4443-a05e-3fd280a2be3a)

![1685606812457](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/52c67a7d-587e-4b59-9e1a-2c8a1903ea7f)

         
 <p><strong>16. CASTING</strong>
         </p>    
<p> Με  βάση όσα έχουν άναφερθει παραπάνω ,δεν υπάρχει casting </p>        
          <p><strong>17. ΛΗΨΕΙΣ</strong>
       
<p>Για αυτήν την διαδίκασια άποφασισαμε οτί οι λήψεις πρέπει να ορίζοντιες για να φαίνεται περισσότερο ο χώρος, αφου τα τόπια είναι το μόναδικο στόιχειο που φαίνεται στην κάμερα .Βγάλαμε παρά πόλλα βίντεο απο τα δίαφορα μέρη που ανάφερονται παράπανω και διάλαξαμε αυτά που πίστευαμε ότι ταιρίαζουν καλύτερα με το περίεχομενο του γράμματος.</p>

</p>    
<p><strong>18. ΣΚΗΝΕΣ</strong>
         
![1_alli](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/b85fea84-e8be-4ac6-abb6-67ecfe7545d0)
![2_alli](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/f5a1ed55-86fa-45dd-8f4a-9dfc9ad22411)
![3_alli](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/5fb992cf-78e0-47af-b13d-57f05b79f7ef)
![5_alli](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/4f06c9d7-7a2d-4f00-b8cf-8ae3a4b75a7d)
![6_alli](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/d9bc5c3b-c79b-47c8-8923-e27d22bb8ad1)

![4_alli](https://github.com/Marina-Eleni-Anastasiadou/Digital-Storytelling-Group-Assignment/assets/100956292/a705ded8-c9f6-40ab-abb4-45d9e96dc02b)


</p>   
 <p><strong>19. EVALUATION</strong>
<p> Για την ταίνια σιγούρα θα προσπάθουθαμε να είχαμε πιο στάθερα πλάνα ίσως και πιο όμορφα.Έπισης θα θέλαμε αν είχαμε χρόνο να τίαχναμε καλύτερα την ποίοτητα τιυ ήχου της ταίνιας μας κατί το όποιο θα ήταν δύνατον αν είχαμ περίσοοτερο χρόνο και καλύτερο μικρόφονο.
</p>         
<p>Όσον αφόρα το επιτράπεζιο θα θέλεμα ίδανικα να το φτλαιχναμε πιο όμορφα δίοτι είχαμε πρόβλημε με τον χρόνο.
</p>   
    <p><strong>20. ΠΗΓΕΣ</strong>
         </p>        
</p>
